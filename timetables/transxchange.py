...
class TransXTimetable(etree._ElementTree):
...
#' ## Methods
    ...
    def get_df(self, dict_of_paths):
        ...
        #Check that data is right length and add blanks if not
        col_lens = [len(i[1]) for i in cols]
        
        for i in cols:
            if len(i[1]) < max(col_lens):
                i[1].extend(['']*(max(col_lens)-len(i[1])))
            
        df = pd.DataFrame.from_dict(dict(cols))
        return df
